        ...
        <div class="col-sm-12">
            <div class="card students">
                <div class="btn-group btn-square">
                    ...
                    <a href="{{ route('student.uneducated') }}" class="btn btn-square btn-primary">O'qimaganlar <span class="badge badge-info">{{ $students_count['uneducated'] }}</span></a>
                </div>
                <div class="card-body" style="position: relative; padding-top: 10px;">
                    <table id="datatableUneducated" class="display bg-info" style="width:100%;">
                        <thead>
                        <tr>
                            <th width="4%">№</th>
                            <th>Ism</th>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var tableUneducated = $('#datatableUneducated').DataTable({
                paging: true,
                pageLength: 20,
                lengthChange: false,
                searching: true,
                ...
                processing: true,
                serverSide: true,
                ajax: '{{ route("student.getUneducated") }}',
                columns: [
                    {data: 'number'},
                    {data: 'firstname'},
                    {data: 'lastname'},
            ...
            tableUneducated.on( 'draw', function () {
            ...
            $(document).on('click', '.js_edit_btn', function () {
                ...
                $.ajax({
                    ...
                    success: (response) => {
                        ...
                        $('#edit_cause').val(response.student.cause)
            ...
            $(document).on('click', '.js_show_btn', function (e) {
                ...
                $.ajax({
                    ...
                    success: (response) => {
                        ...
                        tableStudentShow.find('tbody').append('<tr>\n' +
                                    '                            <th class="text-center">12</th>\n' +
                                    '                            <td class="align-middle">Nima sabab ketgan</td>\n' +
                                    '                            <td class="created_at">'+response[0].cause+'</td>\n' +
                                    '                        </tr>')
                    },
                    error: (response) => {
                        console.log('error:', response)
